...
const manifest_docs = @import("manifest_docs");
...
/// If `decl_handle` has a same-file doctest (`test DeclName { ... }`), append it as a documentation example section.
...
fn findZonManifestField(tree: *const Ast, source_index: usize) ?struct { loc: offsets.Loc, doc: []const u8 } {
    ...
    for (root_struct.ast.fields) |value_node| {
        ...
        if (name_loc.start <= source_index and source_index <= name_loc.end) {
            const doc = manifest_docs.getTopLevel(field_name) orelse return null;
        ...
        for (deps_struct.ast.fields) |dep_value_node| {
            ...
            for (dep_struct.ast.fields) |dep_field_value| {
                ...
                if (dep_field_loc.start <= source_index and source_index <= dep_field_loc.end) {
                    const doc = manifest_docs.getDependencyField(tree.tokenSlice(dep_field_name_token)) orelse return null;
...
/// the field's documentation from the vendored manifest schema (see `lib/manifest`).
